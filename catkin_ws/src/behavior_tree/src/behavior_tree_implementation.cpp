...
FallbackNode::FallbackNode(){
  label = "?";
  is_active = false;
  status = behavior_tree_msgs::Status::FAILURE;
}
...
bool FallbackNode::tick(bool active, int traversal_count){
  ...
  if(!active){
    ...
    for(int i = 0; i < children.size(); i++){
      ...
      if(status == behavior_tree_msgs::Status::FAILURE){
...
bool SequenceNode::tick(bool active, int traversal_count){
  ...
  if(!active){
    ...
    status = behavior_tree_msgs::Status::SUCCESS;
    for(int i = 0; i < children.size(); i++){
      Node* child = children[i];
...
bool ParallelNode::tick(bool active, int traversal_count){
  ...
  if(!active){
    ...
    for(int i = 0; i < children.size(); i++){
      ...
      if(child->status == behavior_tree_msgs::Status::SUCCESS)
	child_success_count++;
      else if(child->status == behavior_tree_msgs::Status::FAILURE)
	child_failure_count++;
    }
    ...
    if(child_success_count >= child_success_threshold)
      ...
      status = behavior_tree_msgs::Status::RUNNING;
  }
...
void ExecutionNode::set_status(uint8_t status){
  this->status = status;
  status_modification_time = ros::Time::now();
}

double ExecutionNode::get_status_age(){
  return (ros::Time::now() - status_modification_time).toSec();
}


ConditionNode::ConditionNode(std::string label){
  this->label = label;
  status_modification_time = ros::Time::now();
  is_active = false;
  status = behavior_tree_msgs::Status::FAILURE;
...
bool ConditionNode::tick(bool active, int traversal_count){
  ...
  if(!is_active && active && status == behavior_tree_msgs::Status::FAILURE)
    set_status(behavior_tree_msgs::Status::FAILURE);
...
void ConditionNode::init_subscriber(){
  ros::NodeHandle nh;
  subscriber = nh.subscribe(get_subscriber_name(), 1, &ConditionNode::callback, this);
}

void ConditionNode::callback(std_msgs::Bool msg){
  //ROS_INFO_STREAM(label << ": " << (int)msg.data);
  // TODO: bug where if status is set here the changed flag won't be set correctly in tick
  uint8_t prev_status = status;
  
  if(msg.data)
    set_status(behavior_tree_msgs::Status::SUCCESS);
  else
    set_status(behavior_tree_msgs::Status::FAILURE);
...
ActionNode::ActionNode(std::string label){
  this->label = label;
  status_modification_time = ros::Time::now();
  is_active = false;
  status = behavior_tree_msgs::Status::FAILURE;
...
bool ActionNode::tick(bool active, int traversal_count){
  ...
  if(!is_active and active){
    set_status(behavior_tree_msgs::Status::RUNNING);
...
void ActionNode::init_publisher(){
  ros::NodeHandle nh;
  publisher = nh.advertise<behavior_tree_msgs::Active>(get_publisher_name(), 1);
...
void ActionNode::init_subscriber(){
  ros::NodeHandle nh;
  subscriber = nh.subscribe(get_subscriber_name(), 1, &ActionNode::callback, this);
}

void ActionNode::callback(behavior_tree_msgs::Status msg){
  uint8_t prev_status = status;
  if(is_active){
...
void ActionNode::publish_active_msg(int active_id){
  if(publisher_initialized){
    behavior_tree_msgs::Active active_msg;
...
BehaviorTree::BehaviorTree(std::string config_filename){
  ...
  ros::NodeHandle nh;
  active_actions_pub = nh.advertise<std_msgs::String>("active_actions", 1);
  graphviz_pub = nh.advertise<std_msgs::String>("behavior_tree_graphviz", 1);
  compressed_pub = nh.advertise<std_msgs::String>("behavior_tree_graphviz_compressed", 1);
...
void BehaviorTree::parse_config(){
  ...
  if(in.is_open()){
	...
	node = new ConditionNode(strip_brackets(label));
      else if(label.compare(0, 1, "[") == 0){
	node = new ActionNode(strip_brackets(label));
...
bool BehaviorTree::tick(){
  ...
  if(root != NULL){
    ...
    std_msgs::String active_actions;
...
std::string BehaviorTree::get_graphviz(){
  ...
  while(!nodes_worklist.empty()){
    ...
    if(node->is_active){
      ...
      if(node->status == behavior_tree_msgs::Status::SUCCESS)
	style += "fillcolor=green";
      else if(node->status == behavior_tree_msgs::Status::RUNNING)
      style += "fillcolor=blue";
      else if(node->status == behavior_tree_msgs::Status::FAILURE)
      style += "fillcolor=red";
    }
    else{
      style += "penwidth=2 ";
